...
describe('showCategory - WALO-45', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('debería mostrar categoría (visible: true)', async () => {
    ...
    vi.mocked(prisma.category.findFirst)
      ...
      .mockResolvedValueOnce({ id: 'cat-1', name: 'Ropa', isActive: true, visible: true } as any) // after update

    vi.mocked(prisma.category.updateMany).mockResolvedValue({ count: 1 } as any)

    const result = await showCategory('cat-1')
  ...
  it('debería rechazar si la categoría no pertenece a la tienda', async () => {
    ...
    const result = await showCategory('cat-999')
